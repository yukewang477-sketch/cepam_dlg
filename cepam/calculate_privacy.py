...
def calculate_epsilon_tilde(epsilon: float, tau: int, dataset_size: int) -> float:
    ...
    epsilon_tilde = np.log(1 + (np.exp(epsilon) - 1)/p)
    return epsilon_tilde

def approximate_epsilon_tilde(epsilon: float, tau: int, dataset_size: int) -> float:
    p = 1 - (1 - 1 / dataset_size)** tau
    epsilon_tilde = epsilon - np.log(p)
    return epsilon_tilde
...
if __name__ == "__main__":
    test_params = {
        ...
        'tau': 15,
        #'K': 30,
        'gamma': 1, #the scale parameter 3/sqrt(N) where N=message size/lattice dimension
        'dataset_size': 1666, #for MNIST is 50000/30=1666 for CIFAR-10 is 40000/30=1333
        #'dataset_size': 1333,
        #'b': 1
    }
    #epsilon = calculate_epsilon_from_b(**test_params)
    ...
    gamma = 1
    dataset_size = 1666
    #epsilon_set = [1,2,3,4,5,10,20,30,40,50,100, 200, 300, 400, 500, 600, 700, 1000, 2000, 3000, 4000, 5000]
    epsilon_set = [1000,2000,3000,4000,5000,6000,7000,8000,9000,1000]
    for epsilon in epsilon_set:
        epsilon_t = calculate_epsilon_tilde(epsilon, tau, dataset_size)
        epsilon_t2 = approximate_epsilon_tilde(epsilon, tau, dataset_size)
        b = 2 * tau * gamma / epsilon_t2
        #print(f"(eps, eps_t, b): ({epsilon}, {epsilon_t}, {b})")
        print(f"(eps, eps_t1, eps_t2, b): ({epsilon}, {epsilon_t}, {epsilon_t2}, {b})")
