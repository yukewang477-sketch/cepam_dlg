...
def train_one_epoch(train_loader, model,
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        #     print(f"Abnormal loss detected: {loss.item()} at batch {batch_idx}")
        #     break
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def compute_gradients(train_loader, model,
                      creterion, device, iterations=None):
    """Compute full-batch gradients of the local objective at the current model weights."""
    model.train()
    grad_buffer = {name: torch.zeros_like(param, device=device) for name, param in model.named_parameters()}
    total_loss = 0.0
    total_samples = 0
    batches_seen = 0
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        for name, param in model.named_parameters():
            grad_buffer[name] += param.grad.detach() * batch_size

        batches_seen += 1
        if iterations is not None and batches_seen == iterations:
            break

    if total_samples == 0:
        return 0.0, grad_buffer

    for name in grad_buffer:
        grad_buffer[name] /= float(total_samples)

    mean_loss = total_loss / max(batches_seen, 1)
    return mean_loss, grad_buffer
