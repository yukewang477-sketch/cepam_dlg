...
def plot_ssim_vs_communication(result_dir=None, lattice_dim_idx=0, b_idx=0, 
    ...
    if vanilla_path and cepam_path:
        ...
        if len(vanilla_ssim.shape) != 2 or len(cepam_ssim.shape) != 2:
            print(f"Error: Unexpected shape for Test 1 format. Expected [communication numbers, images]")
            return
        
        num_communication = vanilla_ssim.shape[0]
        communication = list(range(num_communication))
        ...
        num_communication = ssim_mat.shape[1]
        communication = list(range(num_communication))
    ...
    plt.plot(communication, vanilla_ssim_avg, '-o', linewidth=2.5, markersize=8, 
             label='Vanilla DLG', color='red', alpha=0.8)
    plt.plot(communication, cepam_ssim_avg, '-s', linewidth=2.5, markersize=8, 
             label='CEPAM-Laplace', color='blue', alpha=0.8)
    ...
    plt.xlabel('Communication Number', fontsize=14, fontweight='bold')
    plt.ylabel('SSIM Score', fontsize=14, fontweight='bold')
    plt.title(f'SSIM Score of Vanilla DLG and CEPAM vs Communication Number\n({lattice_label}, {param_label})', 
    ...
    output_filename = current_dir / f'ssim_vs_communication_lattice{lattice_val}_b{param_val_str}.png'
    pdf_filename = current_dir / f'ssim_vs_communication_lattice{lattice_val}_b{param_val_str}.pdf'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Plot SSIM vs Communication for CEPAM DLG experiments',
    ...
    plot_ssim_vs_communication(result_dir=result_dir,
